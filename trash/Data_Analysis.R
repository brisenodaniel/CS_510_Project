...
#	Actual_vs_Pred_dist.png: Actual <- partition $T_c$ distribution plotted
...
control_df <- data.frame(sapply(control, as.numeric))
...
part <-list('train'=train[part_idxs[['train']],], 
            'test'=train[part_idxs[['test']],])
bst <- trainer(part_idxs[['train']],'critical_temp')
test <- part[['test']] %>% select(-critical_temp)
pred <-data.frame( predict_many(bst,part_idxs[['test']]))
